...
public interface GeoRenderer<T extends GeoAnimatable, O, R extends GeoRenderState> {
	...
	default void renderCube(R renderState, GeoCube cube, PoseStack poseStack, VertexConsumer buffer, int packedLight, int packedOverlay, int renderColor) {
		...
		for (GeoQuad quad : cube.quads()) {
			...
			createVerticesOfQuad(renderState, quad, poseState, normalX, normalY, normalZ, buffer, packedOverlay, packedLight, renderColor);
		}
	}
	 ...
	 */
	default void createVerticesOfQuad(R renderState, GeoQuad quad, Matrix4f poseState, float normalX, float normalY, float normalZ, VertexConsumer buffer,
		...
		for (GeoVertex vertex : quad.vertices()) {
			...
			buffer.addVertex(transformedX, transformedY, transformedZ, renderColor, vertex.texU(),
					vertex.texV(), packedOverlay, packedLight, normalX, normalY, normalZ);
		}
	}
